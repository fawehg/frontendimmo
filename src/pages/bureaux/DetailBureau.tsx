...
interface Bureau {
  ...
  adresse: string;
  ville: RelatedEntity | null;
  delegation: RelatedEntity | null;
  categorie: RelatedEntity | null;
  type: RelatedEntity | null;
  environnement: RelatedEntity | null;
...
const DetailBureau = () => {
  ...
  useEffect(() => {
    const fetchBureau = async () => {
      try {
        const response = await axios.get<Bureau>(`http://localhost:8000/api/bureaux/${id}`);
        setBureau(response.data);
        setLoading(false);

        const mockSimilarProperties: SimilarProperty[] = [
          {
            id: 1,
            titre: "Bureau moderne à Sousse",
            prix: 350000,
            adresse: "Sousse",
            ville: "Tunisie",
            superficie: 120,
          ...
          {
            id: 2,
            titre: "Espace professionnel spacieux",
            prix: 420000,
            adresse: "Tunis",
            ville: "Tunisie",
            superficie: 150,
        ...
        setError("Impossible de charger les détails du bureau");
        setLoading(false);
      }
    };
    fetchBureau();
  ...
  const navigateFullScreen = (direction: "prev" | "next") => {
    ...
    if (direction === "prev") {
      setCurrentFullScreenIndex((prev) =>
        prev === 0 ? bureau.images.length - 1 : prev - 1
      );
    } else {
      setCurrentFullScreenIndex((prev) =>
        prev === bureau.images.length - 1 ? 0 : prev + 1
      );
    }
  };
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // e.g., await axios.post(`http://localhost:8000/api/bureaux/${id}/messages`, newMessage);
  ...
  const formatPhoneForLinks = (phone: string | undefined): string => {
    ...
    // Remove spaces, dashes, and other non-digit characters, ensure + is kept
    const cleanedPhone = phone.replace(/[^\d+]/g, "");
    // Ensure the phone starts with a country code (e.g., +216)
    ...
    return (
      ...
      >
        <div className="contenu-non-trouve">
          <h3>Bureau non trouvé</h3>
          <p>Le bureau que vous recherchez n'existe pas ou a été supprimé.</p>
          <Link to="/" className="bouton-retour-accueil">
            <FaArrowLeft /> Retour à l'accueil
          </Link>
        </div>
    ...
    <div className="detail-bureau">
      ...
      >
        ...
        <div
          ...
          style={{
            backgroundImage: `url(${bureau.images[0]?.url || "https://via.placeholder.com/1920x1080"})`,
        ...
        <div className="contenu-parallaxe">
          ...
          >
            <FaMapMarkerAlt />
            <span>
              {bureau.adresse}, {bureau.ville?.nom || "Non défini"} -{" "}
              {bureau.delegation?.nom || "Non défini"}
            </span>
          </motion.div>
        ...
        >
          <span className="prix">{bureau.prix.toLocaleString()} TND</span>
          <span className="type-transaction">{bureau.type?.nom || "Non défini"}</span>
        </motion.div>
      </motion.section>
        ...
        <div className="conteneur-galerie">
          <div className="galerie-principale">
            <AnimatePresence mode="wait">
              ...
              >
                <motion.img
                  src={bureau.images[activeImageIndex]?.url || "https://via.placeholder.com/800x450"}
                  alt={`${bureau.titre} ${activeImageIndex + 1}`}
            ...
            <div className="controles-galerie">
              <motion.button
                ...
                onClick={() =>
                  handleThumbnailClick(
                    activeImageIndex === 0 ? bureau.images.length - 1 : activeImageIndex - 1
                  )
                }
              ...
              <motion.button
                ...
                onClick={() =>
                  handleThumbnailClick(
                    activeImageIndex === bureau.images.length - 1 ? 0 : activeImageIndex + 1
                  )
                }
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.9 }}
              ...
              <div className="visuel-caracteristiques">
                <div className="grille-visuelle">
                  <div className="element-visuel">
                    <CercleProgression
                      value={Math.min(bureau.superficie / 2, 100)}
                      label="Superficie"
                      icon={<FaRulerCombined />}
                      count={`${bureau.superficie}m²`}
                    />
                  </div>
                  <div className="element-visuel">
                    <CercleProgression
                      value={bureau.nombre_bureaux * 10}
                      label="Bureaux"
                      icon={<FaBriefcase />}
                      count={bureau.nombre_bureaux}
                    />
                  </div>
                  <div className="element-visuel">
                    <CercleProgression
                      value={bureau.nombre_toilettes * 20}
                      label="Toilettes"
                      icon={<FaToilet />}
                      count={bureau.nombre_toilettes}
                    />
                  </div>
                </div>
              </div>
              <div className="details-caracteristiques">
                <div className="rangee-caracteristiques">
                  ...
                  <div className="caracteristique">
                    ...
                    <div className="info-caracteristique">
                      <span className="etiquette-caracteristique">Nombre de bureaux</span>
                      <span className="valeur-caracteristique">{bureau.nombre_bureaux}</span>
                    </div>
                  </div>
                </div>
                <div className="rangee-caracteristiques">
                  <div className="caracteristique">
                    ...
                    <div className="info-caracteristique">
                      <span className="etiquette-caracteristique">Nombre de toilettes</span>
                      <span className="valeur-caracteristique">{bureau.nombre_toilettes}</span>
                    </div>
                  </div>
                  {bureau.caracteristiques && bureau.caracteristiques.length > 0 && (
                    <div className="caracteristique pleine-largeur">
                      <div className="icone-caracteristique">
                        <FaTree />
                      </div>
                      <div className="info-caracteristique">
                        <span className="etiquette-caracteristique">Caractéristiques</span>
                        <span className="valeur-caracteristique">
                          {bureau.caracteristiques.map(c => c.nom).join(", ")}
                        </span>
                      </div>
                    </div>
                  )}
            ...
            <div className="contenu-carte">
              <div className="mini-carte">
                <div className="apercu-carte">
                  <iframe
                    src={`https://maps.google.com/maps?q=${bureau.ville?.nom || "Tunisie"},${bureau.delegation?.nom || "Tunisie"}&output=embed`}
                    frameBorder="0"
                    allowFullScreen
                  ></iframe>
                </div>
              ...
              <div className="elements-info">
                <div className="element-info">
                  ...
                  <div className="texte-info">
                    ...
                    <p>{bureau.type?.nom || "Non spécifié"}</p>
                  </div>
                </div>
                <div className="element-info">
                  <div className="icone-info">
                  ...
                  <div className="texte-info">
                    <h4>Catégorie</h4>
                    <p>{bureau.categorie?.nom || "Non spécifié"}</p>
                  </div>
                </div>
                <div className="element-info">
                  <div className="icone-info">
                  ...
                  <div className="texte-info">
                    <h4>Localisation</h4>
                    <p>
                      {bureau.delegation?.nom || "Non spécifié"}, {bureau.ville?.nom || "Non spécifié"}
                    </p>
                  </div>
                </div>
                <div className="element-info">
                  ...
                  <div className="texte-info">
                    <h4>Environnement</h4>
                    <p>{bureau.environnement?.nom || "Non spécifié"}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="contenu-carte">
              <div className="profil-agent">
                ...
                <div className="details-agent">
                  <h3>{bureau.vendeur ? `${bureau.vendeur.prenom} ${bureau.vendeur.nom}` : "Vendeur non spécifié"}</h3>
              ...
              <div className="methodes-contact">
                <motion.div className="methode-contact" whileHover={{ y: -5 }}>
                  <a
                    href={`tel:${formatPhoneForLinks(bureau.vendeur?.phone)}`}
                    target="_blank"
                  ...
                  >
                    ...
                    <div className="info-methode">
                      <h4>Appelez-moi</h4>
                      <p>{bureau.vendeur?.phone || "+216 98 765 432"}</p>
                    </div>
                  </a>
                </motion.div>
                <motion.div className="methode-contact" whileHover={{ y: -5 }}>
                  <a
                    href={`https://wa.me/${formatPhoneForLinks(bureau.vendeur?.phone)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                ...
                <motion.div className="methode-contact" whileHover={{ y: -5 }}>
                  <a
                    href={`mailto:${formatEmailForMailto(bureau.vendeur?.email)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                  ...
                  >
                    ...
                    <div className="info-methode">
                      <h4>Email</h4>
                      <p>{bureau.vendeur?.email || "contact@immobilier.tn"}</p>
                    </div>
                  </a>
                </motion.div>
              </div>
        ...
        {showFullScreen && (
          ...
          >
            ...
            <div className="conteneur-image-plein-ecran">
              <motion.img
                src={bureau.images[currentFullScreenIndex]?.url || "https://via.placeholder.com/1200x800"}
