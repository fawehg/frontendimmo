...
const Home: React.FC = () => {
  ...
  const { scrollYProgress } = useScroll({
    target: recentPropertiesRef,
    offset: ["start end", "end start"],
  });

  const y = useTransform(scrollYProgress, [0, 1], ["0%", "10%"]);

  // Scroll to card in explore section
  ...
  // Handle scroll for back-to-top and nav visibility
  useEffect(() => {
    const handleScroll = () => {
  ...
  const scrollToProperties = () => {
    if (recentPropertiesRef.current) {
      recentPropertiesRef.current.scrollIntoView({ behavior: "smooth" });
    }
  };
  ...
  useEffect(() => {
    ...
    setProperties(filteredProperties.slice(0, 6)); // Limit to 6 properties for home page
  }, [maisons, appartements, villas, bureaux, fermes, terrains, etageVillas, filters]);

  // Fetch delegations based on ville selection
              ...
              {showFilters && (
                <div className="advanced-filters">
                  ...
                  <div className="form-group-roww">
                    <button type="button" className="search-button" onClick={scrollToProperties}>
