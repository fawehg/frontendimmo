...
import resetImage from "../../assets/reset.avif"; 
...
const ResetPassword = () => {
  ...
  const requestResetCode = async (data: any) => {
    try {
      const response = await axios.post<ApiResponse>("http://localhost:8000/api/reset-password", data);
  ...
  const verifyCodeAndResetPassword = async (data: any) => {
    try {
      const response = await axios.post<ApiResponse>("http://localhost:8000/api/verify-reset-code", { ...data, email });
      alert(response.data.ResultData.message);
      reset();
      setStep(1);
      navigate("/login"); 
            ...
            {step === 1 ? (
              <form onSubmit={handleSubmit(requestResetCode)}>
                ...
                <p className="reset-info-text">
                  Vous recevrez un code de réinitialisation par email.
                </p>
              </form>
            ) : (
              <form onSubmit={handleSubmit(verifyCodeAndResetPassword)}>
                ...
                <button type="submit" className="reset-btn reset-btn-success">
